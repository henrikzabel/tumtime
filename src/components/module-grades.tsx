...
import {
  ...
  formatSemesterShort,
  type Semester,
} from "@/lib/stats/semester";
import { cn } from "@/lib/utils";

const SOURCE_LABELS: Record<string, string> = {
  tum_info: "TUM Info",
  aamin: "stats.aamin.dev",
...
function trendData(
  exams: ExamWithGrades[],
  metric: "averageTotal" | "failureRate",
): TrendPoint[] {
  const bySemester = new Map<string, TrendPoint & { key: number }>();
  for (const e of exams) {
    const point = bySemester.get(e.semester) ?? {
      label: shortSemester(e.semester),
      key: e.semesterKey,
    };
...
/**
 * Exam statistics of one module: headline figures, trends and one card per exam.
 * `filterHref` builds the link for the endterm/retake filter (keeps other params of the page).
 */
export function ModuleGrades({
  exams,
  type,
  filterHref,
}: {
  ...
  filterHref: (type: string | undefined) => string;
}) {
  ...
  const newestFirst = [...visible].sort(
    (a, b) => b.semesterKey - a.semesterKey || a.type.localeCompare(b.type),
  );
  const latestEndterm =
    [...exams].reverse().find((e) => e.type === "endterm") ?? exams.at(-1)!;
  ...
  return (
    <>
      <section className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <StatTile
          label="Failure rate"
          value={formatPercent(latestEndterm.failureRate)}
          hint="of students who took the exam"
        />
        <StatTile
          label="Participants"
          value={formatCount(latestEndterm.attempted)}
          hint="attempts in that exam"
        />
        <StatTile
          label="Exams on record"
          value={String(exams.length)}
          hint={`${shortSemester(exams[0].semester)} – ${shortSemester(exams.at(-1)!.semester)}`}
        />
      </section>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2">
          <Card className="gap-3">
            <CardHeader>
              ...
              <CardDescription>
                Lower is better — the axis is flipped so better years sit
                higher.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <TrendChart
                data={trendData(exams, "averageTotal")}
                metric="grade"
                domain={[1, 5]}
                reversed
              />
          ...
          <Card className="gap-3">
            <CardHeader>
              ...
              <CardDescription>
                Share of participants who failed.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <TrendChart
                data={trendData(exams, "failureRate")}
                metric="percent"
                domain={[0, 1]}
              />
            </CardContent>
      ...
      <p className="mt-10 text-xs text-muted-foreground">
        ...
        Aggregated statistics only. Distributions of exams with fewer than{" "}
        {MIN_ATTEMPTS_FOR_DISTRIBUTION} participants are hidden.
      </p>
    </>
  );
}
      ...
      <CardHeader>
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-base">
            {formatSemester(e.semester as Semester)}
          </CardTitle>
          <Badge variant={e.type === "endterm" ? "secondary" : "outline"}>
            {e.type === "endterm" ? "Endterm" : "Retake"}
          </Badge>
        </div>
        {date || extras.length ? (
          <CardDescription>
            {[date, ...extras].filter(Boolean).join(" · ")}
          </CardDescription>
        ) : null}
      </CardHeader>
